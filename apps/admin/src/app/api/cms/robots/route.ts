...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  if (!staff) return NextResponse.redirect(new URL("/cms/seo/robots", request.url), { status: 303 });
  ...
  return NextResponse.redirect(new URL("/cms/seo/robots", request.url), { status: 303 });
}
