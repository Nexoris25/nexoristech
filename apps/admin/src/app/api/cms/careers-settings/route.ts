...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  if (!staff) return NextResponse.redirect(new URL("/cms/careers/settings", request.url), { status: 303 });
  ...
  await cmsDb().query(
    ...
    [data, staff.name]);
  return NextResponse.redirect(new URL("/cms/careers/settings", request.url), { status: 303 });
}
