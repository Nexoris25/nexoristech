...
export default async function LegalPagesPage(): Promise<ReactNode> {
  ...
  const { rows } = await cmsDb().query<Row>(
    `SELECT id, title, slug, version, status, COALESCE(updated_at, created_at)::text AS updated_at
       FROM cms_content WHERE kind='legal_page' ORDER BY title LIMIT 50`);
    ...
    <div>
      ...
      <div className="mt-5 rounded-2xl border border-slate-200 bg-white shadow-subtle">
        <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 p-3">
          <div className="relative min-w-0 flex-1 sm:max-w-sm"><Search size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" /><input placeholder="Search legal pages..." className="w-full rounded-lg border border-slate-200 bg-slate-50 py-2 pl-9 pr-3 text-[0.83rem] focus:border-[#543CDA] focus:bg-white focus:outline-none" /></div>
        </div>
        ...
        <div className="border-t border-slate-100 px-5 py-3 text-[0.8rem] text-slate-500">
          <span>Showing 1 to {rows.length} of {rows.length} legal pages</span>
        </div>
      </div>
    </div>
  );
