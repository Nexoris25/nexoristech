/**
 * The NRS e-Invoicing module gate.
 *
 * Every page below this point is behind it, which is the point: guarding pages one at a time is how
 * the e-invoicing area ended up with eleven pages with no guard on them at all. A layout runs for
...
export default async function EInvoicingLayout({ children }: { children: ReactNode }): Promise<ReactNode> {
  await requireModule("einvoicing");
  return <>{children}</>;
}
