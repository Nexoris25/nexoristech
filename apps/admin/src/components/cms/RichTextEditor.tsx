...
export function RichTextEditor({ name, initialHtml, onChange, registerApi }: { name: string; initialHtml?: string; onChange?: (html: string) => void; registerApi?: (api: RichTextApi) => void }): ReactNode {
  ...
  const onPaste = (e: ClipboardEvent<HTMLDivElement>): void => {
    ...
    e.preventDefault();
    const clean = html ? normaliseHtml(html) : normalisePlainText(text);
    if (!clean) return;
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white">
      ...
      <div ref={ref} contentEditable suppressContentEditableWarning role="textbox" aria-multiline="true"
        ...
        className="cms-rte min-h-[420px] px-5 py-4 text-[0.92rem] leading-relaxed text-slate-800 focus:outline-none"
